function getJson(str) {
  ...
  str = str.replaceAll(/"\s*,\s*"/g, `":"`).replaceAll('\n', '').replace(/\s+$/, '')
...
function getValueChunks(json, chunkSize = 100) {
  let list = []
  for (const dict of Object.values(json)) {
    list = [...list, ...Object.values(dict)]
  }
...
function getKeyChunks(json, chunkSize = 100) {
  let list = []
  for (const dict of Object.values(json)) {
    list = [...list, ...Object.keys(dict)]
  }
...
function mergeChunks(chunks) {
  ...
  for (const chunk of chunks) {
    list = [...list, ...JSON.parse(chunk)]
  }
  return list
}

function LanguageHandler(cSharp) {
  this.json = getJson(cSharp)
  this.keyChunks = getKeyChunks(this.json)
  this.valueChunks = getValueChunks(this.json)
  this.translatedChunks = []

  this.setTranslatedChunks = (chunkNumber, translated) => {
    this.translatedChunks[chunkNumber] = translated
  }

  this.getTranslatedCSharp = () => {
    if (
      this.keyChunks.length !== this.valueChunks.length ||
      this.translatedChunks.length !== this.valueChunks.length
    ) {
      console.log(this.keyChunks, this.translatedChunks)
      throw new Error('Exception during parse.')
    }

    var parsedKeys = mergeChunks(this.keyChunks)
    var parsedTranslated = mergeChunks(this.translatedChunks)
    for (let i = 0; i < this.keyChunks.length; i++) {
      if (parsedKeys.length != parsedTranslated.length) {
        throw new Error('Invalid translated chunks: may lack of something.')
      }
    }

    let cSharpList = ''
    for (let i = 0; i < parsedKeys.length; i++) {
      cSharpList += `{ "${parsedKeys[i]}" , "${parsedTranslated[i]}" }, `
    }

    return cSharpList
  }

  this.getValueChunks = () => this.valueChunks
}

// let a = new LanguageHandler('{ "test", "t" } , { "test2", "t2" }, ')
// a.setTranslatedChunks(0, '["测试"]')
// a.setTranslatedChunks(1, '["测试2"]')
// a.getTranslatedCSharp()
...
const str = `      { "HelloWorld", "Hello, World!" },
      ...
      { "UI/FightReadyGuide/Page0/0", "Here will show the attributes of the monsters in the current map. Remember these attributes and match your own strategy to defeat the monsters!" },
      { "UI/FightReadyGuide/Page1/0", "These two positions will show the bosses sitting in the map. Each boss has different combat skills and styles. Try to use different techniques to defeat different bosses!" },
      { "UI/FightReadyGuide/Page2/0", "Don't forget to explore the map more often, where you can collect different gems to strengthen your heroes.\nYou can also use the honors you get from battles to unlock new heroes and runes for better exploration in different maps!" },
      ...
      { "UI/Info/Unlock/InsufficientBalance", "Sorry, insufficient balance to unlock." },
      ...
      { "UI/Hero/Description/Witch", "The Arcanist unleashes multiple balls of lightning to attack the enemy." },
      {
      ...
      { "Upgrade/IncreaseAttackingSpeed/Name", "Hit&Run Lv{0}" },
      { "Upgrade/IncreaseAttackingSpeed/Description", "Increase attacking speed by {0:F2}%." },
      ...
      { "Upgrade/AddPuppetLv1/Name", "Mafia Lv{0}" },
      {
        ...
        "Summon more puppets for group fights, {0} puppets every {1:F2} seconds."
      },
      ...
      { "Upgrade/ActivateMine/Name", "Landmine" },
      {
        ...
        "Plant {0} mines that explodes and causes {1:F2} damage within a {2:F2} meter radius every {3:F2} seconds."
      },
      ...
      },
      {
        ...
        "The hero's attack is reduced by 5%, and an additional 5% is reduced for each level."
      },
      {
...
const tr = new LanguageHandler(str)
tr.getValueChunks()